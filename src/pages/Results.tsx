...
import { Scale, TrendingUp, MapPin, Users, Heart, Globe, History, Telescope, AlertTriangle, CheckCircle2, MinusCircle, Share2, Copy } from "lucide-react";
...
const Results = () => {
  ...
  if (modules.modLegal) {
    ...
    cards.push({
      id: "legal", title: "Simplified Legal Overview", icon: Scale,
      status: ai?.status || "positive",
      content: lang === "hi" && ai?.summary_hi ? <p className="font-hindi">{ai.summary_hi}</p> : <p>{ai?.summary_en || "This draft law simplifies GST input credit for micro and small enterprises, limits overlapping local taxes, and gives districts with weaker digital infrastructure more time and support."}</p>,
  ...
  if (modules.modGeo) {
    ...
    cards.push({
      id: "geo", title: role === "central" ? "State-wise Impact Overview" : `District-wise Impact: ${state}`, icon: MapPin,
      status: ai?.status || "neutral",
      content: (
        <div className="space-y-2">
          {role === "central" ? (
            <>
              <div className="flex items-center gap-2 p-2 bg-success/10 rounded"><div className="w-3 h-3 rounded-full bg-success" /><span>{ai?.highReadiness?.join(", ") || "Gujarat, Maharashtra, Karnataka"} - High digital readiness</span></div>
              <div className="flex items-center gap-2 p-2 bg-warning/10 rounded"><div className="w-3 h-3 rounded-full bg-warning" /><span>{ai?.mediumReadiness?.join(", ") || "Rajasthan, MP, Odisha"} - Medium infrastructure</span></div>
              <div className="flex items-center gap-2 p-2 bg-destructive/10 rounded"><div className="w-3 h-3 rounded-full bg-destructive" /><span>{ai?.needSupport?.join(", ") || "Bihar, Jharkhand, Chhattisgarh"} - Need extended support</span></div>
            </>
          ) : (
            <p>Districts in {state} show varying readiness levels. Urban districts likely to adopt quickly; rural districts may need the full 18-month compliance window.</p>
          )}
        </div>
      ),
  ...
  if (modules.modCommunity) {
    ...
    cards.push({
      id: "community", title: "Community Impact Analysis", icon: Users,
      status: ai?.status || "neutral",
      content: (
        <div className="space-y-2">
          <p>{ai?.summary || "Tribal artisan cooperatives in scheduled areas may face initial documentation challenges."}</p>
          {ai?.recommendations && <ul className="list-disc list-inside text-sm space-y-1">{ai.recommendations.map((r, i) => <li key={i}>{r}</li>)}</ul>}
        </div>
      ),
      ...
      content: (
        <div className="space-y-2">
          {ai?.comparisons ? (
            ...
            <><p>Similar policies in Singapore and UK showed:</p><ul className="list-disc list-inside text-sm space-y-1"><li>15-20% increase in formal sector registration</li><li>Reduced compliance burden by approximately 25%</li></ul></>
          )}
        </div>
      ),
      ...
      content: (
        <div className="space-y-2">
          ...
          <ul className="list-disc list-inside text-sm space-y-1">
            {(ai?.lessons || ["Early grievance redressal reduces resistance", "Phased implementation prevents system overload", "Clear communication in vernacular languages essential"]).map((lesson, i) => <li key={i}>{lesson}</li>)}
          </ul>
        </div>
      ),
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-sm text-muted-foreground">
                {role === "central" ? "Central Government view: comparing impact across states." : `State Government view: deep-dive into districts of ${state}.`}
              </p>
